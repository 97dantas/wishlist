...
exports.createCustomer = async (body) => {
    try {
        ...
        logger.info({ endpoint: 'customer/', method: 'createCustomer', request: body, response: resp })

        return resp
    } catch (err) {
        if (err.message.startsWith('E11000')) throw error('E11000')

        logger.error({ endpoint: 'customer/', method: 'createCustomer', err: String(err), request: body })
